...
struct info *readline(ifstream *fbin, FILE *ftxt, int filetype)
{
	...
	if (filetype == 0) {
		...
		if (fscanf(ftxt, "Flow>%04u ", &(*infos_loc).flowId) != EOF) {
			if(fscanf(ftxt, "Seq>%7u ", &(*infos_loc).seqNum) != EOF) {
#ifdef V23
				if(fscanf(ftxt, "Src>%1c%15c/%u ", &buffer, (*infos_loc).srcAddr,&(*infos_loc).srcPort) != EOF){
					if(fscanf(ftxt, "Dest>%1c%15c/%u ", &buffer, (*infos_loc).destAddr,&(*infos_loc).destPort) != EOF){
						if(fscanf(ftxt, "TxTime>%ld:%ld:%lf ", &(*infos_loc).txTime1, &(*infos_loc).txTime2,&(*infos_loc).txTime3) != EOF){
							if(fscanf(ftxt, "RxTime>%ld:%ld:%lf ", &(*infos_loc).rxTime1, &(*infos_loc).rxTime2,&(*infos_loc).rxTime3) != EOF){
#else
#ifdef V21
				if(fscanf(ftxt, "Src>%1c%15c/%u ", &buffer, (*infos_loc).srcAddr,&(*infos_loc).srcPort) != EOF){
					if(fscanf(ftxt, "Dest>%1c%15c/%u ", &buffer, (*infos_loc).destAddr,&(*infos_loc).destPort) != EOF){
						if(fscanf(ftxt, "TxTime>%ld:%ld:%lf ", &(*infos_loc).txTime1, &(*infos_loc).txTime2,&(*infos_loc).txTime3) != EOF){
							if(fscanf(ftxt, "RxTime>%ld:%ld:%lf ", &(*infos_loc).rxTime1, &(*infos_loc).rxTime2,&(*infos_loc).rxTime3) != EOF){
#else
				if(fscanf(ftxt, "Src>%1c%45c/%u ", &buffer, (*infos_loc).srcAddr, &(*infos_loc).srcPort) != EOF){
					if(fscanf(ftxt, "Dest>%1c%45c/%u ", &buffer, (*infos_loc).destAddr, &(*infos_loc).destPort) != EOF){
						if(fscanf(ftxt, "txTime>%u:%u:%lf ", &(*infos_loc).txTime1, &(*infos_loc).txTime2, &(*infos_loc).txTime3) != EOF){
							if(fscanf(ftxt, "rxTime>%u:%u:%lf ", &(*infos_loc).rxTime1, &(*infos_loc).rxTime2, &(*infos_loc).rxTime3) != EOF){
#endif
#endif
								if(fscanf(ftxt, "Size>%5u\n", &(*infos_loc).size) != EOF){
	...
	while (1) {
		...
		while (memFile[i].flowId != (unsigned int) -1) {
			...
			if (fscanf(memFile[i].tempfile_bit, "%lf %lf\n", &time, &value) == EOF) {
				...
				value = -1;
				if(fscanf(memFile[i].tempfile_del, "%lf %lf\n", &time, &value) != EOF){
					valuesum_del += value;

				
					value = -1;
					if(fscanf(memFile[i].tempfile_jit, "%lf %lf\n", &time, &value) != EOF){
						valuesum_jit += value;
				
				
						value = -1;
						if(fscanf(memFile[i].tempfile_pkt, "%lf %lf\n", &time, &value) != EOF){
							valuesum_pkt += value;
						}
					}
				}				
